...
#### Inputs/Outputs:
Use return values when possible over output parameters, this improves readability. Avoid returning pointers unless its possible for them to be null, preferably return by value, failing tha, return by reference. When having non-optional input parameters they should be constant references or values, while output or input/output ones should generally be references. 
#### Style:
